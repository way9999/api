...
builtins.yasal_bypass_ip_choice = True  # 容器模式：默认绕过 IP 检查

# 允许的出口地区白名单（环境变量，逗号分隔，留空则不限制）
ALLOWED_REGIONS = [
    r.strip().upper()
    for r in os.environ.get("ALLOWED_REGIONS", "").split(",")
    if r.strip()
]
...
def run(
    ...
    try:
        ...
        # 地区白名单过滤（优先级最高）
        if ALLOWED_REGIONS and loc and loc.upper() not in ALLOWED_REGIONS:
            print(
                f"[线程 {thread_id}] [Yasal 换路中~] {loc} 不在允许地区 {ALLOWED_REGIONS} 内，换个节点重试~"
            )
            return None
    ...
    while True:
        ...
        try:
            ...
            if result:
                ...
                print(
                    f"[线程 {thread_id}] Token 已保存: {file_name}"
                )

                # 自动注入到 CLIProxyAPI auth 目录
                try:
                    from inject import inject_token
                    inject_token(file_name)
                except Exception as ie:
                    print(f"[线程 {thread_id}] 自动注入失败: {ie}")
